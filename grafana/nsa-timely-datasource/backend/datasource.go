...
func (td *TimelyDatasource) CallResource(ctx context.Context, request *backend.CallResourceRequest, sender backend.CallResourceResponseSender) error {
	...
	if strings.HasPrefix(request.Path, "/api/aggregators") ||
		...
		if err != nil {
			logger.Error("Error fetching resource", "url", url.String(), "error", err)
			return err
		}
		if httpResponse == nil {
			logger.Error("Error fetching resource", "url", url.String())
			return fmt.Errorf("Error fetching resource url:%s", url.String())
		}

		if httpResponse.StatusCode != 200 {
			logger.Error("Error fetching resource", "url", url.String(), "responseCode", httpResponse.StatusCode)
			return fmt.Errorf("Error fetching resource url:%s responseCode:%d", url.String(), httpResponse.StatusCode)
		}

		bytes, err := io.ReadAll(httpResponse.Body)
		if err != nil {
			logger.Error("Error reading resource", "url", request.URL, "error", err)
			return err
		}
		response.Status = 200
		response.Body = bytes
	...
	if queryDataRequest.PluginContext.User == nil {
		...
		if err == nil {
			timelyQueryForm = TimelyQueryForm{
				...
				RateInterval:         timelyQueryServer.Interval,
